...
class cAlgoIndex(alg.cAlgoBase):
    ...
    def indexCalc(self):
        ...
        if self.getOfferPrice(usd) != 0 and self.getBidPrice(index) != 0:
            self.availableBid = round(min(self.getBidPrice(index)*self.getBidSize(index)/self.getOfferPrice(usd)/1000,
        ...
        if self.getBidPrice(usd) != 0 and self.getOfferPrice(index) != 0:
            self.availableOffer = round(min(self.getOfferPrice(index)*self.getOfferSize(index)/self.getBidPrice(usd)/1000,
